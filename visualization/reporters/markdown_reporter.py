...
class MarkdownReporter:
    ...
    def generate_comparison_table(
        ...
        for idx in range(len(methods)):
            ...
            if highlight_idx is not None and idx == highlight_idx:
                method = f"**{method}**"
                row = (
                    f"| {method} | {backbones[idx]} | {params[idx]} | {flops[idx]} | **{one_shot[idx]}** | **{five_shot[idx]}** |"
                )
            else:
                row = (
                    f"| {method} | {backbones[idx]} | {params[idx]} | {flops[idx]} | {one_shot[idx]} | {five_shot[idx]} |"
    ...
    def generate_ablation_table(
        ...
        lines = [
            "## Ablation Study",
            "",
            "| Config | SSEG | NASE | Curr. | Dist. | 1-shot | 5-shot |",
            "| :--- | :---: | :---: | :---: | :---: | :---: | :---: |",
            "",
        ]
        for idx in range(len(configs)):
            ...
            row = (
                f"| {configs[idx]} | {sseg_str} | {nase_str} | {curr_str} | {dist_str} | {one_shot[idx]} | {five_shot[idx]} |"
            )
            lines.append(row)
        return "\n".join(lines)
