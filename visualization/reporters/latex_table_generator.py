...
class LatexTableGenerator:
    ...
    def generate_main_results_table(
        ...
        for row in other_rows:
            line = (
                f"{row.method} & {row.backbone} & {row.params} & {row.flops} & {row.one_shot} & {row.five_shot} \\\\"
            )
            lines.append(line)
        if ours_rows:
            ...
            for row in ours_rows:
                line = (
                    f"\\textbf{{{row.method}}} & {row.backbone} & {row.params} & {row.flops} & \\textbf{{{row.one_shot}}} & \\textbf{{{row.five_shot}}} \\\\"
                )
                lines.append(line)
        lines.extend([
            r"\bottomrule",
    ...
    def generate_ablation_table(
        ...
        lines = [
            ...
            r"\toprule",
            f"Configuration & {header_components} & 1-shot & 5-shot \\\\",
            r"\midrule",
        ]
    ...
    def generate_efficiency_table(
        ...
        lines = [
            ...
            r"\toprule",
            r"Method & Params & FLOPs & Time (ms) & Acc/GFLOP \\",
            r"\midrule",
        ]
        for idx, method in enumerate(methods):
            line = (
                f"{method} & {params[idx]} & {flops[idx]} & {inference_time[idx]} & {accuracy_per_flop[idx]} \\\\"
            )
            lines.append(line)
        lines.extend([
            r"\bottomrule",
