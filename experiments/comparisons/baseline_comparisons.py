...
class BaselineResult: 
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "flops": self.flops,
            "flops_G": self.flops_giga,
            "one_shot": self.one_shot_accuracy,
            "one_shot_margin": self.one_shot_margin,
            "five_shot": self.five_shot_accuracy,
            "five_shot_margin": self.five_shot_margin,
            "acc_per_flop": self.accuracy_per_flop,
...
class BaselineComparison:
    ...
    def generate_comparison_table(self) -> str:
        lines = [
            ...
            "",
            "| Method | Backbone | Params | FLOPs | 1-shot | 5-shot | Acc/GFLOP |",
            "| :--- | :--- | :---: | :---: | :---: | :---: | :---: |",
            "",
        ]
        all_results = []
        for baseline in self._baseline_results:
            all_results.append({
                ...
                "efficiency": f"{baseline.accuracy_per_flop:.2f}",
                "is_ours": False,
            })
        if self._ours_result:
            all_results.append({
                "method": f"**{self._ours_result.method_name}**",
                ...
                "efficiency": f"**{self._ours_result.accuracy_per_flop:.2f}**",
                "is_ours": True,
            })
        for res in all_results: 
            line = (
                f"| {res['method']} | {res['backbone']} | {res['params']} | {res['flops']} | {res['one_shot']} | {res['five_shot']} | {res['efficiency']} |"
            )
            lines.append(line)
        return "\n".join(lines)
    def generate_latex_table(self) -> str:
        lines = [
            ...
            r"\toprule",
            r"Method & Backbone & Params & FLOPs & 1-shot & 5-shot & Acc/G \\",
            r"\midrule",
        ]
        for baseline in self._baseline_results:
            line = (
                f"{baseline.method_name} & {baseline.backbone} & {baseline.params_millions:.2f}M & {baseline.flops_giga:.2f}G & {baseline.one_shot_accuracy:.2f} & {baseline.five_shot_accuracy:.2f} & {baseline.accuracy_per_flop:.2f} \\\\"
            )
            lines.append(line)
        lines.append(r"\midrule")
        if self._ours_result:
            line = (
                f"\\textbf{{{self._ours_result.method_name}}} & {self._ours_result.backbone} & \\textbf{{{self._ours_result.params_millions:.2f}M}} & \\textbf{{{self._ours_result.flops_giga:.2f}G}} & \\textbf{{{self._ours_result.one_shot_ci.mean:.2f}}} & \\textbf{{{self._ours_result.five_shot_ci.mean:.2f}}} & \\textbf{{{self._ours_result.accuracy_per_flop:.2f}}} \\\\"
            )
            lines.append(line)
        lines.extend([
            r"\bottomrule",
    ...
    def save_results(self) -> None:
        ...
        json_data = {
            ...
            "ours": {
                "method": self._ours_result.method_name,
                "backbone": self._ours_result.backbone,
                ...
                "acc_per_flop": self._ours_result.accuracy_per_flop,
