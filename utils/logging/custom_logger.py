...
class CustomLogger:
    ...
    def log_architecture(
        ...
        message = (
            f"Architecture: blocks={num_blocks}, params={num_params:,}, features={feature_dim}"
        )
        self.info(message)
    def log_training_step(
        ...
        message = (
            f"Epoch [{epoch}] Step [{step}/{total_steps}] Loss: {loss:.4f} LR: {lr:.6f}"
        )
        self.info(message)
    def log_epoch_summary(
        ...
        message = (
            f"Epoch {epoch} Summary: SSL Loss={ssl_loss:.4f}, Distill Loss={distill_loss:.4f}, Level={curriculum_level}"
        )
        self.info(message)
    def log_evolution_event(
        ...
        message = (
            f"Evolution: {mutation_type} on layer {target_layer}. Params: {params_before} -> {params_after} (Delta: {delta})"
        )
        self.info(message)
    def log_evaluation_result(
        ...
        message = (
            f"Evaluation ({num_shots}-shot): Accuracy={mean_accuracy:.2f}% +/- {margin:.2f}%"
        )
        self.info(message)
