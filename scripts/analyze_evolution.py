...
def load_evolution_data(checkpoint_dir: Path) -> dict:
    ...
    if len(evolution_data["epochs"]) < 2:
        ...
        arch_log = log_dir / "architecture_evolution.txt"
        if arch_log.exists():
            print(f"DEBUG: Checkpoints sparse, falling back to {arch_log}")
            with open(arch_log, "r") as f:
                content = f.read()
            evolution_data["epochs"] = []
            evolution_data["num_params"] = []
            evolution_data["num_blocks"] = []
            import re
            epoch_blocks = re.split(r"Epoch (\d+):", content)[1:] 
            for i in range(0, len(epoch_blocks), 2):
                ...
                params_match = re.search(r"Total Params:\s+([0-9,]+)", block_content)
                if params_match:
                    params_cleaned = params_match.group(1).replace(",", "")
                    evolution_data["num_params"].append(int(params_cleaned) / 1e6)
                blocks_match = re.search(r"Num Blocks:\s+(\d+)", block_content)
                if blocks_match:
                    evolution_data["num_blocks"].append(int(blocks_match.group(1)))
